...
/usr/sbin/tcpdump -Annr $fileToProcess | egrep "\b((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\.|$)){4}\b" -o | cut -d\. -f1,2,3,4 | sort | uniq -c | sort -nr > $objPath/$currentDate/$currentDate.IP.histogram.txt
/usr/sbin/tcpdump -Annr $fileToProcess | grep "User-Agent" | sort | uniq -c | sort -nr > $objPath/$currentDate/$currentDate.UA.histogram.txt
/usr/sbin/tcpdump -Annr $fileToProcess | egrep "\:\/\/([^\/\"]{2,100})" -o | cut -d\/ -f3 | sort | uniq -c | sort -nr > $objPath/$currentDate/$currentDate.Domains.histogram.txt
